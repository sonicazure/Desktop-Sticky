...
- **置顶 / 置底**：置顶时浮在所有窗口之上；关闭置顶则自动置底——压到 Z 序最底，不遮挡之后打开的窗口
- **Win+D 防埋**：非置顶模式下，被桌面图层压盖（如 Win+D「显示桌面」）时自动浮回桌面之上、应用窗口之下，不会弹到最前遮挡其他程序
...
| `dockguard.py` | 桌面防埋守卫：非置顶时防 Win+D「显示桌面」把组件压到桌面层下 |
